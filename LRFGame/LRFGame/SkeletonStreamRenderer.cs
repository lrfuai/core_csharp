...
namespace LRFGame
{
    ...
    public class SkeletonStreamRenderer : Object2D
    {
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            if (null == this.Follower.Skeltons || null == this.mapMethod)
            {
                return;
            }
            ...
            foreach (var skeleton in this.Follower.Skeltons)
            {
                switch (skeleton.TrackingState)
                {
                    case SkeletonTrackingState.Tracked:
                        // Draw Bones
                        this.DrawID(skeleton, JointType.ShoulderLeft);
        ...
        private void DrawID(Skeleton skel, JointType aJointType)
        {
            Vector2 start = this.mapMethod(skel.Joints[aJointType].Position);
            Vector2 place = new Vector2(start.X, start.Y);
            this.SharedSpriteBatch.DrawString(this.font,skel.TrackingId.ToString(),place, Color.Red);
        }
    }
}
